...
php_darwin_die() {
  if [ -n "${PHP_DARWIN_PHASE:-}" ]; then
    php_darwin_log_metric failure.phase "$PHP_DARWIN_PHASE"
  fi
  php_darwin_show_metrics >&2
  if [ -n "${PHP_DARWIN_PHASE:-}" ]; then
...
php_darwin_log_metric() {
  [ -n "${PHP_DARWIN_TIMING_LOG:-}" ] || return 0
  printf '%s=%s\n' "$1" "$2" >> "$PHP_DARWIN_TIMING_LOG" 2>/dev/null || true
}

php_darwin_show_metrics() {
  [ -n "${PHP_DARWIN_TIMING_LOG:-}" ] && [ -s "$PHP_DARWIN_TIMING_LOG" ] || return 0
  while IFS= read -r php_darwin_metric; do
    printf 'php-darwin: timing %s\n' "$php_darwin_metric"
  done < "$PHP_DARWIN_TIMING_LOG"
}
...
php_darwin_validate_version() {
  ...
  done < "$php_darwin_root/conf/versions"
...
php_darwin_postinstall_paths() {
  ...
  done < "$php_darwin_root/conf/postinstall-paths"
}
