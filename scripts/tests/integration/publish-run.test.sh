...
PHP_DARWIN_RUN_JSON="$run_json" bash "$script_dir/../../release/validate-publish-run.sh" 123 >/dev/null || \
...
PHP_DARWIN_RUN_JSON="$run_json.publish-failed" bash "$script_dir/../../release/validate-publish-run.sh" 123 \
...
if PHP_DARWIN_RUN_JSON="$run_json.in-progress" bash "$script_dir/../../release/validate-publish-run.sh" 123 \
  >/dev/null 2>&1; then
...
if PHP_DARWIN_RUN_JSON="$run_json.failed" bash "$script_dir/../../release/validate-publish-run.sh" 123 \
  >/dev/null 2>&1; then
...
if PHP_DARWIN_RUN_JSON="$run_json.failed-build" bash "$script_dir/../../release/validate-publish-run.sh" 123 \
  >/dev/null 2>&1; then
...
if PHP_DARWIN_RUN_JSON="$run_json.untested" bash "$script_dir/../../release/validate-publish-run.sh" 123 \
  >/dev/null 2>&1; then
