...
verify_cache() {
  ...
  local actual_pear_path
  local asset
  local checksum
  local contents
  local embedded_metadata
  local expected_pear_path
  local invalid_path
  ...
  expected_pear_path=$(php_darwin_pear_path "$version" "$formula")
  actual_pear_path=$(jq -er '.pear_path | select(type == "string")' "$metadata") || \
    php_darwin_die 'archive metadata PEAR path is missing or invalid'
  [ "$actual_pear_path" = "$expected_pear_path" ] || \
    php_darwin_die "archive metadata PEAR path is $actual_pear_path; expected $expected_pear_path"
  jq -e --arg archive "$asset" --arg arch "$arch" --arg build "$build" \
    --arg commit "$source_commit" --arg formula "$formula" --arg php "$version" \
    --arg pear_path "$expected_pear_path" \
