...
prepare_homebrew() {
  ...
  brew untap --force "$tap" || php_darwin_die "could not remove $tap before testing the cached snapshot"
...
install_cache() {
  local elapsed
  local installer_elapsed
  local max_install_seconds
  ...
  max_install_seconds=$(php_darwin_package_config max_install_seconds)
  installer_elapsed=$(awk -F= '$1 == "installer.total_seconds" { value=$2 } END {
    if (value !~ /^[0-9]+$/) exit 1
    print value
  }' "${PHP_DARWIN_TIMING_LOG:?}") || php_darwin_die 'cache installation timing metric is missing'
  wall_limit=$((max_install_seconds + 1))
  printf 'Cache installation completed in %s seconds; installer reported %s seconds (target: %s, runner tolerance: %s)\n' \
    "$elapsed" "$installer_elapsed" "$max_install_seconds" "$wall_limit"
  [ "$installer_elapsed" -le "$wall_limit" ] || \
    php_darwin_die "cache installer reported $installer_elapsed seconds; expected at most $wall_limit with runner tolerance"
  [ "$elapsed" -le "$wall_limit" ] || \
    php_darwin_die "cache installation process took $elapsed seconds; expected at most $wall_limit with clock tolerance"
}
...
validate_runtime() {
  ...
  expected_pear_dir="$brew_prefix/share/pear"
  [[ "$formula" = php@* ]] && expected_pear_dir="$expected_pear_dir@$version"
  ...
  php -r "if (strpos(PHP_VERSION, '$version') !== 0) { exit(1); }" || \
    php_darwin_die 'linked PHP does not match the requested version'
  "$php_bin" -v || php_darwin_die 'php -v failed'
  "$php_bin" -m || php_darwin_die 'php -m failed'
  "$php_config" --version || php_darwin_die 'php-config failed'
  "$php_fpm" -v || php_darwin_die 'php-fpm failed'
  php_info=$("$php_bin" -i) || php_darwin_die 'php -i failed'
...
validate_homebrew() {
  ...
  comm -13 "$installed_before" "$installed_after" > "$new_formulae" || \
  ...
  "$php_bin" -r "if (strpos(PHP_VERSION, '$version') !== 0) { exit(1); }" || \
    php_darwin_die 'PHP failed after Homebrew relinking'

  brew services start "$formula" || php_darwin_die 'PHP service did not start'
  sleep 2
  brew services info "$formula" --json | jq -e '.[0].running == true' >/dev/null || \
    php_darwin_die 'PHP service is not running'
...
case "$stage" in
  ...
  all)
    ...
    validate_homebrew
    ;;
  *) php_darwin_die 'usage: test-install.sh prepare|install|runtime|homebrew|all' ;;
esac
