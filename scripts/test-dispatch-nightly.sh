...
bash "$script_dir/dispatch-nightly.sh" || php_darwin_die 'nightly dispatch failed'
...
if PHP_DARWIN_TEST_GH_STATUS=1 bash "$script_dir/dispatch-nightly.sh" >/dev/null 2>&1; then
...
for configuration in 'stable 8.5' 'nightly 8.7 unexpected'; do
  ...
  : > "$PHP_DARWIN_TEST_GH_LOG"
  if PHP_DARWIN_ROOT="$work_dir" bash "$script_dir/dispatch-nightly.sh" >/dev/null 2>&1; then
