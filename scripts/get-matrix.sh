...
for build in "${build_values[@]}"; do
  for ts in "${ts_values[@]}"; do
    for requested_arch in "${arch_values[@]}"; do
      ...
      jq -cn --arg php "$php_version" --arg build "$build" --arg ts "$ts" --arg arch "$arch" --arg runner "$runner" \
        '{php:$php,build:$build,ts:$ts,arch:$arch,runner:$runner}' >> "$build_entries_file"
      ...
      while IFS= read -r test_runner; do
        jq -cn --arg php "$php_version" --arg build "$build" --arg ts "$ts" --arg arch "$arch" --arg runner "$test_runner" \
          '{php:$php,build:$build,ts:$ts,arch:$arch,runner:$runner}' >> "$test_entries_file"
