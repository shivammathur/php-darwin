...
package_cache() {
  ...
  source_hash=$(HOMEBREW_PHP_PATH="$tap_path" bash "$script_dir/../lib/source-hash.sh" "$version") || \
...
verify_cache() {
  ...
  bash "$script_dir/../installer/read-metadata.sh" "$output" "$(php_darwin_metadata_path "$asset")" \
