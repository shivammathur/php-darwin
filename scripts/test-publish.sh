...
while read -r build ts extra; do
  ...
  for arch in arm64 x86_64; do
    ...
    jq --arg archive "$asset" --arg architecture "$arch" --arg brew_prefix "$prefix" \
      ...
      .state_paths=[("etc/php/" + $php_version + "/pear.conf")] | .tap_snapshot=$tap_snapshot |
...
: > "$gh_log"
HOMEBREW_PHP_COMMIT="$source_commit" PATH="$fake_bin:$PATH" bash "$script_dir/publish.sh" "$builds_dir" >/dev/null || \
...
grep -Eq '^release view php-8\.5 ' "$gh_log" || php_darwin_die 'publisher did not inspect the minor release'
grep -Eq '^release create php-8\.5 ' "$gh_log" || php_darwin_die 'publisher did not create the minor release'
grep -Eq '^release upload php-8\.5 .*--clobber' "$gh_log" || \
  php_darwin_die 'publisher did not replace the deterministic release assets'
jq -e --arg source_hash "$source_hash" '
  .schema == 1 and .php_version == "8.5" and .php_semver == "8.5.10" and
